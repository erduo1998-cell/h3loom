...
H3Loom 是独立的工作流与部署工具。自有代码、三个 Skill 和文档文字采用 [Apache-2.0](LICENSE)，版权归属见 [NOTICE](NOTICE)。本许可不覆盖模型、上游代码、NVIDIA 二进制文件或个人身份素材；各自适用条款和原始声明继续保留。
...
- 本项目没有取得可替代模型条款的授权，不能以代码许可证推断模型在全球或所有用途均可自由使用。
